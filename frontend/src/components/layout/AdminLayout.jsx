...
import {
  ...
  Avatar, Badge, Popover, CircularProgress, Button, Chip
...
import { getMyNotifications, getUnreadCount, markAsRead, markAllAsRead } from '../../services/notificationApi';
...
export default function AdminLayout() {
  ...
  const getNotifColor = (type) => {
    switch (type) {
      case 'new_booking': return '#2b6f56';
      case 'assignment': return '#1976d2';
      case 'booking_confirmed': return '#4caf50';
  ...
  const menuItems = [
    ...
    { text: 'Quản lý Booking', icon: <ConfirmationNumberIcon />, path: '/manager/bookings' },
    { text: 'Quản lý Lịch', icon: <CalendarMonthIcon />, path: '/manager/schedules' },
    { text: 'Quản lý Tour', icon: <ExploreIcon />, path: '/manager/tours' },
    { text: 'Quản lý Hang Động', icon: <LandscapeIcon />, path: '/manager/caves' },
    { text: 'Thư viện ảnh', icon: <CollectionsIcon />, path: '/manager/media' },
    { text: 'Quản lý Tài khoản', icon: <PeopleIcon />, path: '/manager/accounts' },
  ];

  const drawer = (
    <div>
      <Toolbar>
        <Typography variant="h6" noWrap component="div" sx={{ fontWeight: 'bold', color: '#2b6f56' }}>
          OXALIS ADMIN
        </Typography>
      </Toolbar>
      <Divider />
      <List>
        {menuItems.map((item) => (
          <ListItem key={item.text} disablePadding>
            ...
            >
              <ListItemIcon sx={{ color: location.pathname === item.path ? '#2b6f56' : 'inherit' }}>
                {item.icon}
              </ListItemIcon>
              <ListItemText
                primary={item.text}
                primaryTypographyProps={{
                  ...
                  color: location.pathname === item.path ? '#2b6f56' : 'inherit'
                }}
              />
            </ListItemButton>
          </ListItem>
    ...
    <Box sx={{ display: 'flex', minHeight: '100vh', backgroundColor: '#f8fafc' }}>
      ...
      >
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', p: 2, borderBottom: '1px solid #eee' }}>
          <Typography fontWeight="bold" fontSize="1rem">Thông báo</Typography>
          {unreadCount > 0 && (
            <Button size="small" startIcon={<DoneAllIcon />} onClick={handleMarkAllRead} sx={{ textTransform: 'none', fontSize: '0.8rem' }}>
              Đọc tất cả
            </Button>
          )}
        </Box>
        {notifLoading ? (
          ...
          <Box sx={{ py: 4, textAlign: 'center' }}><Typography color="#999">Không có thông báo</Typography></Box>
        ) : (
          <Box sx={{ maxHeight: 380, overflowY: 'auto' }}>
            {notifications.map(n => (
              ...
              >
                <Box sx={{ width: 8, height: 8, borderRadius: '50%', bgcolor: n.isRead ? 'transparent' : getNotifColor(n.type), mt: 0.8, flexShrink: 0 }} />
                <Box sx={{ flex: 1, minWidth: 0 }}>
                  <Typography fontSize="0.85rem" fontWeight={n.isRead ? 400 : 700} noWrap>{n.title}</Typography>
                  <Typography fontSize="0.78rem" color="#666" sx={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                    {n.content}
                  </Typography>
