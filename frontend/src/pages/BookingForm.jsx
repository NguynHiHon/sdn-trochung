...
const BookingForm = ({ tourId, selectedDate, onBookingSuccess, onCancel }) => {
    ...
    const handleHold = async (e) => {
        ...
        try {
            ...
            const response = await fetch('http://localhost:5000/api/booking/hold', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    // need auth token if required 'Authorization': `Bearer ...`
                },
                body: JSON.stringify({
    ...
    const handleConfirm = async () => {
        ...
        setError(null);
        try {
            const response = await fetch(`http://localhost:5000/api/booking/confirm/${holdData._id}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
