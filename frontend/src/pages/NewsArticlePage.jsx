...
export default function NewsArticlePage() {
  ...
  const blogCategoryPath = "/news/category/our-blog";
  ...
  return (
    <Box sx={{ bgcolor: "#f5f2eb", minHeight: "70vh" }}>
      ...
      <Container maxWidth="lg" sx={{ py: { xs: 4, md: 6 } }}>
        <Box sx={{ width: "100%", maxWidth: 760, mx: "auto", px: { xs: 1, sm: 0 } }}>
        ...
        >
          <Typography component={Link} to="/news" sx={{ color: "#2b6f56", textDecoration: "none", fontSize: "inherit", fontWeight: "inherit" }}>
        ...
        <Box sx={{ width: "100%", maxWidth: 760, mx: "auto", overflowX: "hidden" }}>
          {isHtml ? (
            <Box
        ...
        {related.length > 0 && (
          <Box sx={{ maxWidth: 1060, mx: "auto", mt: { xs: 6, md: 8 } }}>
            ...
            <Typography sx={{ fontWeight: 800, color: "#1b3026", mb: 2.5, fontSize: { xs: "1.4rem", md: "1.8rem" } }}>
              {lang === "vi" ? "Khám phá thêm" : "Discover More"}
            </Typography>
            <Grid container spacing={3}>
              {related.map((item) => (
                <Grid item xs={12} md={4} key={item._id}>
                  <Card elevation={0} sx={{ borderRadius: "4px", overflow: "hidden", bgcolor: "#fff", height: "100%" }}>
                    <CardActionArea component={Link} to={`/news/article/${item.slug}`} sx={{ height: "100%" }}>
                      <Box sx={{ width: "100%", aspectRatio: "16 / 9", bgcolor: "#e8e4dc" }}>
                        {item.thumbnail?.url ? (
                          <Box
                            component="img"
                      ...
                      <CardContent>
                        <Typography sx={{ fontWeight: 700, lineHeight: 1.35, color: "#1d1d1d", mb: 1 }}>
                          {item.title?.[lang]}
                        </Typography>
                        <Typography sx={{ color: "rgba(0,0,0,0.6)", fontSize: "0.92rem", lineHeight: 1.6, display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                          {item.excerpt?.[lang] || "\u00a0"}
                        </Typography>
                      </CardContent>
                    </CardActionArea>
