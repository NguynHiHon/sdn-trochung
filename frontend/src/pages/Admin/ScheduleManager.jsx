...
export default function ScheduleManager() {
  ...
  const submitAssignGuide = async () => {
    ...
    try {
      ...
      if (res.success) {
        toast.success('Đã gán trưởng đoàn');
    ...
    <Box>
      ...
      <Dialog open={openBulk} onClose={() => setOpenBulk(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent dividers>
          ...
          <Box sx={{ display: 'flex', gap: 1, mb: 2 }}>
            <TextField type="date" size="small" fullWidth value={tempDate} onChange={e => setTempDate(e.target.value)}
      ...
      <Dialog open={Boolean(assignDialog)} onClose={() => setAssignDialog(null)} maxWidth="sm" fullWidth>
        ...
        <DialogContent dividers>
          ...
          >
            <MenuItem value="">-- Bỏ gán --</MenuItem>
            {staffList.map((s) => (
              <MenuItem key={s._id} value={s._id}>
                {s.fullName || s.username} {s.email ? `(${s.email})` : ''}
              </MenuItem>
            ))}
          </TextField>
        </DialogContent>
